...
class LockingTests(BaseTestCase):
    ...
    def setUp(self):
        try:
            ts_dweepy.remove_lock(test_lock, test_key)
        except ts_dweepy.DweepyError:
            pass
    ...
    def test_remove_lock_locked(self):
        ...
        ts_dweepy.lock(my_thing_id, test_lock, test_key)
        self.assertEqual(ts_dweepy.remove_lock(test_lock, test_key), test_lock)
    ...
    def test_lock_unlock(self):
        ...
        my_thing_id = str(uuid.uuid4())
        ts_dweepy.lock(my_thing_id, test_lock, test_key)
        ts_dweepy.unlock(my_thing_id, test_key)
...
class LockedTests(BaseTestCase):
    ...
    def setUp(self):
        ...
        ts_dweepy.lock(self.my_thing_id, test_lock, test_key)

    def tearDown(self):
        ts_dweepy.unlock(self.my_thing_id, test_key)
        try:
            ts_dweepy.remove_lock(test_lock, test_key)
        except ts_dweepy.DweepyError:
            pass
    ...
    def test_get_latest_dweet_for_without_a_key(self):
        ...
        """
        try:
            ts_dweepy.get_latest_dweet_for(self.my_thing_id)
        except ts_dweepy.DweepyError as e:
            self.assertEqual(e.args[0], 'this thing is locked and requires a key')
        else:
            self.fail("shouldn't ever get called")
    ...
    def test_get_latest_dweet_for_with_an_invalid_key(self):
        ...
        """
        try:
            ts_dweepy.get_latest_dweet_for(self.my_thing_id, key='badkey')
        except ts_dweepy.DweepyError as e:
            self.assertEqual(e.args[0], 'the key you provided doesn\'t work with this thing')
        else:
            self.fail("shouldn't ever get called")
...
class LockedStreamingDweetsTests(BaseTestCase):
    ...
    def test_listen_for_dweets_from_with_key(self):
        ...
        """
        dweets_heard = 0
        for dweet in ts_dweepy.listen_for_dweets_from(self.my_thing_id, timeout=5, key=test_key):
            dweets_heard += 1
            check_valid_dweet_response(self, dweet)
...
class AlertTests(BaseTestCase):
    ...
    def test_get_alert(self):
        ...
        alert = ts_dweepy.get_alert(self.my_thing_id, test_key)
        self.assertEqual(alert['condition'], test_alert_condition)
    ...
    def test_dweet_for_alert(self):
        ...
        dweet = ts_dweepy.dweet_for(self.my_thing_id, {'alertValue': 11}, key=test_key)
        check_valid_dweet_response(self, dweet, skip_content=True)
        dweet = ts_dweepy.dweet_for(self.my_thing_id, {'alertValue': 5}, key=test_key)
        check_valid_dweet_response(self, dweet, skip_content=True)
        dweet = ts_dweepy.dweet_for(self.my_thing_id, {'alertValue': 10}, key=test_key)
        check_valid_dweet_response(self, dweet, skip_content=True)
    ...
    def test_remove_alert(self):
        ...
        ts_dweepy.remove_alert(self.my_thing_id, test_key)
    ...
    def test_404_after_remove_alert(self):
        ...
        ts_dweepy.remove_alert(self.my_thing_id, test_key)
        try:
            ts_dweepy.get_alert(self.my_thing_id, test_key)
        except ts_dweepy.DweepyError as e:
...
class UnlockedDweetingTests(BaseTestCase):
    ...
    def test_valid_dweet(self):
        ...
        """
        dweet = ts_dweepy.dweet(test_data)
        check_valid_dweet_response(self, dweet)
...
class UnlockedReadingDweetsTests(BaseTestCase):
    ...
    def test_get_dweets_for(self):
        ...
        dweets = ts_dweepy.get_dweets_for(self.my_thing_id)
        check_valid_get_response(self, dweets)
...
class UnlockedStreamingDweetsTests(BaseTestCase):
    ...
    def test_listen_for_dweets_from(self):
        ...
        """
        dweets_heard = 0
        for dweet in ts_dweepy.listen_for_dweets_from(self.my_thing_id, timeout=5):
            dweets_heard += 1
            check_valid_dweet_response(self, dweet)
        self.assertGreater(dweets_heard, 0)
